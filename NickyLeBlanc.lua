...
function LeBlanc:OnTick()
    ...
    if GetSpellNameByIndex(myHero.Addr, _R) == "LeblancRE" then
        self.E.range = 1000
        self.E:SetSkillShot(0.25, math.huge, 30, false)
        self.EUlt = true
    end
...
function LeBlanc:GetELinePreCore(target)
	local castPosX, castPosZ, unitPosX, unitPosZ, hitChance, _aoeTargetsHitCount = GetPredictionCore(target.Addr, 0, self.E.delay, self.E.width, self.E.range, self.E.speed, myHero.x, myHero.z, false, true, 1, 1, 5, 5, 5, 5)
	if target ~= nil then
		 CastPosition = Vector(castPosX, target.y, castPosZ)
		 HitChance = hitChance
		 Position = Vector(unitPosX, target.y, unitPosZ)
...
function LeBlanc:IsUnderTurretEnemy(pos)			--Will Only work near myHero
...
function LB_CastW()
    if self.W:IsReady() and self.WReturn then
        CastSpellTarget(myHero.Addr, _W)
    end 
end 

function LB_CastW2()
    if self.R:IsReady() and self.RWReturn then
...
function LeBlanc:LB_IsEnemy()
    for i,hero in pairs(GetEnemyHeroes()) do
        if hero ~= 0 then
            target = GetAIHero(hero)
            if IsDead(target.Addr) then
                wUsed()
            end 
        end 
    end 
